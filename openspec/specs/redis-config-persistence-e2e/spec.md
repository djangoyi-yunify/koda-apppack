...
本能力定义针对 Redis 7.2.14 配置文件持久化机制的最小化 e2e 验证要求，覆盖 `include` 隔离性、副本配置持久化以及 Sentinel 拓扑持久化。

## Requirements

### Requirement: Redis `include` 隔离性
Redis 运行时主文件通过 `include` 引入只读模板后，`CONFIG REWRITE` 必须只修改运行时主文件，不得修改模板文件。
...
使用与复制无关的简单参数 `maxmemory` 单独验证 `CONFIG REWRITE` 的写文件边界，确保变更只写入运行时主文件，不污染静态模板。
...
副本实例的运行时配置文件必须持久保存 `replicaof` 与 `masterauth`，以便进程重启后恢复正确角色。

#### Scenario: 通过 redis-cli 建立复制关系后配置落盘
- **WHEN** 副本实例以不包含 `replicaof` 的运行时配置文件启动
- **AND** 使用 `redis-cli` 执行 `REPLICAOF` 指向主库
- **AND** 执行 `CONFIG REWRITE`
- **THEN** 副本运行时配置文件包含 `replicaof` 与 `masterauth`

#### Scenario: 副本重启后恢复角色
- **WHEN** 副本实例以已持久化的、包含 `replicaof` 和 `masterauth` 的运行时配置文件启动
- **AND** 主库可达
- **THEN** 副本以 `role:slave` 启动并连接主库

### Requirement: Redis 故障转移后角色持久化
故障转移导致原副本提升为主库后，其运行时配置文件必须被更新以反映新角色；原主库重新加入后必须被改写为副本配置。

#### Scenario: 提升后的副本保持主库角色
- **WHEN** Sentinel 触发故障转移，原副本被提升为主库
- **AND** 新主库上执行 `CONFIG REWRITE`
- **THEN** 新主库的运行时配置文件中不再包含指向旧主库的 `replicaof`
